  ...
  try {
    ...
    if (!userExists) {
      ...
      if (confirmationCode === userExists.confirmationCode) {
        try {
          await userExists.updateOne({ check: true }); // en el caso de esta query, apunta a un elemento concreto (no al modelo)
  ...
  try {
    ...
    transporter.sendMail(mailOptions, async function (error, info) {
      if (error) {
        ...
        const newPasswordBcrypt = bcrypt.hashSync(passwordSecure, 10); //hassyn le digo contraseña que quiero hasear y cuantas vueltas le quiero dar
        ...
        try {
          ...
          }); /**con el meetodo finBy... lo que hace es buscar por id
          y actualizarlo y le digo, encuentramelo por id, que es el que yo recibo por el param que tenemos arriba,
          y le mando un objeto que es: en la clave password me vas a poner la nueva contraseña encryptada.*/
    ...
    return next(error); //aquí va a ser el error general, un 500.
  }
};
...
const modifyPassword = async (req, res, next) => {
   ...
   * 2) _id => lo saco de la req.user (--- lo creamos en auth.middleware )
   * 3) la antigua contraseña que has escrito coincida con el back compareSync (la del back está encryptada, por eso uso compareSync)
   * 4) La nueva contraseña se encrypta
   * 5) se actualiza en el back
   * test----para comprobar que se ha actualizado.
   *
   *
   * Errores:
 ...
 * 1. Cosas que NO queremos que actualice el usuario pero:
  ...
  - confirmationCode
   ...
   2.Cosas que podemos cambiar:
   
  - body: name, image, gender

  3. tenemos un token -> sacamos el _id del usuario gracias a la req.user
  IMPORTANTE recordar que el id del usuario lo sacamos de la clave que creamos nueva en el middelware,
  con la req.user

  4. Con esto ya podemos hacer la query: findByIdAndUpdate
  
  5. Hacemos el test para ver si se ha actualizado.

  ERRORES

  - que no recibamos el token, por lo cual lo vamos a echar por el middleware de auth.
no vamos a tener más errores, porque como no tenemos ningún tipo de enum no hay que valorar si lo
que hemos recibido, esté dentro de la numeración de valores o no.

 */

const update = async (req, res, next) => {
  // capturamos la imagen nueva subida a cloudinary
  let catchImg = req.file?.path; //vamos a tener dos middelware: el de subida de fichero y el de autentificación.

  try {
    // actualizamos los elementos unique del modelo
    await User.syncIndexes();

    // instanciamos un nuevo objeto del modelo de user con el req.body  ----------------!¿POR QUË?
    ...
    try {
       ...
       * del req.body
       * --> el RUNTIME es el tiempo real de ejecución de la aplicación.
       */
      const updateUser = await User.findById(req.user._id);

      /** sacamos las claves del objeto del req.body para saber que info nos han pedido actualizar */
      const updateKeys = Object.keys(req.body); // ["name", "image"]  oObjetc.keys es un método y te saca las claves.
 ...
 * La diferencia que tiene con el find, es que éste es un array vacío si no encuentra nada,
 *  pero el findById si no lo encuentra nos devuelve null
 */
...
const byId = async (req, res, next) => {
  try {
    ...
    if (!mongoose.Types.ObjectId.isValid(req.params.id)) {
      return res.status(400).json({ error: "id no tiene formato valido" });
    }
    const userById = await User.findById(req.params.id); // si no lo encuentra es un null
    if (userById) {
      return res.status(200).json(userById); // también podría ponerlos así .json({data:})
    } else {
...
const getAll = async (req, res, next) => {
  try {
    ...
    if (getAll.length === 0) {
      ...
      return res
        .status(200)
        .json({ data: getAllUser, count: getAllUser.length });
  } catch (error) {
    return next(error);
  }
};
